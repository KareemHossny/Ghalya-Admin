  ...
  const OrderDetailsModal = ({ order, onClose }) => {
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-2 sm:p-4 z-50" onClick={onClose}>
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[95vh] overflow-y-auto mx-2" onClick={(e) => e.stopPropagation()}>
          {/* Header */}
          <div className="bg-gradient-to-r from-pink-500 to-purple-600 p-4 sm:p-6 text-white rounded-t-2xl">
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <h2 className="text-lg sm:text-2xl font-bold truncate">تفاصيل الطلب #{order._id.slice(-8)}</h2>
                <div className="flex flex-wrap items-center gap-2 mt-2">
                  <span className={`px-2 py-1 rounded-full text-xs font-semibold bg-white/20 backdrop-blur-sm ${
              ...
              <button
                onClick={onClose}
                className="p-1 sm:p-2 hover:bg-white/20 rounded-xl transition-all duration-200 flex-shrink-0 ml-2"
              >
          ...
          <div className="p-3 sm:p-6 space-y-4 sm:space-y-6">
            ...
            <div className="bg-gradient-to-l from-pink-50 to-purple-50 rounded-2xl p-3 sm:p-5 border border-pink-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="bg-white p-2 sm:p-3 rounded-xl border border-gray-200">
                  <label className="block text-xs font-semibold text-gray-500 mb-1">الاسم</label>
                  <p className="text-gray-900 font-bold text-sm sm:text-base truncate">{order.customerName}</p>
                </div>
                <div className="bg-white p-2 sm:p-3 rounded-xl border border-gray-200">
                  <label className="block text-xs font-semibold text-gray-500 mb-1">الهاتف</label>
                  <p className="text-gray-900 font-bold text-sm sm:text-base direction-ltr text-left truncate">{order.customerPhone}</p>
                </div>
                <div className="bg-white p-2 sm:p-3 rounded-xl border border-gray-200">
                  <label className="block text-xs font-semibold text-gray-500 mb-1">المحافظة</label>
                  <p className="text-gray-900 font-bold text-sm sm:text-base truncate">{getGovernorateName(order.governorate)}</p>
                </div>
                <div className="sm:col-span-2 bg-white p-2 sm:p-3 rounded-xl border border-gray-200">
                  <label className="block text-xs font-semibold text-gray-500 mb-1">العنوان</label>
                  <p className="text-gray-900 font-bold text-sm sm:text-base break-words">{order.customerAddress}</p>
                </div>
              </div>
            </div>

            {/* Order Items */}
            <div className="bg-white rounded-2xl p-3 sm:p-5 border border-gray-200">
              <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-3 sm:mb-4 flex items-center gap-2">
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gradient-to-r from-gray-50 to-gray-100">
                      <tr>
                        <th className="px-2 sm:px-4 py-2 sm:py-3 text-right text-xs font-bold text-gray-600 uppercase tracking-wider">
                          المنتج
                        </th>
                        <th className="px-2 sm:px-4 py-2 sm:py-3 text-right text-xs font-bold text-gray-600 uppercase tracking-wider">
                          السعر
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {order.items.map((item, index) => (
                        <tr key={index} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-2 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm font-bold text-gray-900 whitespace-nowrap">
                          ...
                          <td className="px-2 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm font-bold text-green-600 whitespace-nowrap">
            ...
            {/* Additional Notes */}
            {order.notes && (
              <div className="bg-yellow-50 rounded-2xl p-3 sm:p-5 border border-yellow-200">
                ...
                <div className="bg-white p-2 sm:p-3 rounded-xl border border-yellow-200">
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        {orders.length === 0 && !apiLoading ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-500">لم يتم تقديم أي طلبات حتى الآن</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {orders.map((order) => (
                  <tr key={order._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 sm:px-6 py-3 whitespace-nowrap text-xs sm:text-sm font-semibold text-gray-900 truncate max-w-[80px] sm:max-w-none">
                    ...
                    <td className="px-4 sm:px-6 py-3 whitespace-nowrap">
                      <div className="flex items-center">
                        <select
                          ...
                          className={`text-xs sm:text-sm font-semibold rounded-xl px-2 py-1 sm:px-3 sm:py-2 border-0 focus:ring-2 focus:ring-pink-500 transition-colors ${
                    ...
                    <td className="px-4 sm:px-6 py-3 whitespace-nowrap text-xs sm:text-sm text-gray-500 font-medium">
                      {new Date(order.orderDate).toLocaleDateString('ar-EG')}
                    </td>
                    <td className="px-4 sm:px-6 py-3 whitespace-nowrap text-xs sm:text-sm font-semibold">
                      <button
                        onClick={() => setSelectedOrder(order)}
                        className="text-pink-600 hover:text-pink-700 ml-2 sm:ml-4 transition-colors"
                      >
                        التفاصيل
                      </button>
                    </td>
