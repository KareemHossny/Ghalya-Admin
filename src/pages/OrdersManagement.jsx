        ...
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="bg-gradient-to-r from-pink-500 to-purple-600 p-6 text-white rounded-t-2xl">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold">تفاصيل الطلب #{order._id.slice(-8)}</h2>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="bg-gradient-to-l from-pink-50 to-purple-50 rounded-2xl p-5 border border-pink-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white p-3 rounded-xl border border-gray-200">
                  ...
                  <p className="text-gray-900 font-bold text-base">{order.customerName}</p>
                </div>
                <div className="bg-white p-3 rounded-xl border border-gray-200">
                  <label className="block text-xs font-semibold text-gray-500 mb-1">الهاتف</label>
                  <p className="text-gray-900 font-bold text-base direction-ltr text-left">{order.customerPhone}</p>
                </div>
                <div className="bg-white p-3 rounded-xl border border-gray-200">
                  <label className="block text-xs font-semibold text-gray-500 mb-1">المحافظة</label>
                  <p className="text-gray-900 font-bold text-base">{getGovernorateName(order.governorate)}</p>
                </div>
                <div className="md:col-span-2 bg-white p-3 rounded-xl border border-gray-200">
                  <label className="block text-xs font-semibold text-gray-500 mb-1">العنوان</label>
                  <p className="text-gray-900 font-bold text-base">{order.customerAddress}</p>
                </div>
              </div>
            </div>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {order.items.map((item, index) => (
                      <tr key={index} className="hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            <img
                              ...
                              className="w-10 h-10 object-cover rounded-lg ml-3 border border-gray-200"
                              onError={(e) => {
                        ...
                        <td className="px-4 py-3">
                          <span className="bg-gray-100 text-gray-800 px-2 py-1 rounded-lg text-sm font-bold">
                            {item.quantity}
                          </span>
                        </td>
            ...
            <div className="bg-gradient-to-l from-pink-50 to-purple-50 rounded-2xl p-5 border border-pink-100">
              ...
              <div className="space-y-3 bg-white p-4 rounded-xl border border-gray-200">
                <div className="flex justify-between items-center py-2">
                  ...
                  <span className="text-gray-900 font-bold">{subtotal} ج.م</span>
                </div>
                <div className="flex justify-between items-center py-2">
                  <span className="text-gray-600 font-semibold">مصاريف الشحن:</span>
                  <span className="text-gray-900 font-bold">{order.shippingCost} ج.م</span>
                </div>
                <div className="flex justify-between items-center pt-3 border-t border-gray-200">
                  <span className="text-lg font-bold text-gray-800">الإجمالي:</span>
                  <span className="text-xl font-bold text-pink-600">{order.totalAmount} ج.م</span>
                </div>
              </div>
            </div>
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-2 text-lg">عرض وإدارة جميع طلبات العملاء</p>
        </div>
      </div>
                  ...
                  <tr key={order._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <select
                        ...
                        className={`text-sm font-semibold rounded-xl px-3 py-2 border-0 focus:ring-2 focus:ring-pink-500 transition-colors ${
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold">
                      <button
                        ...
                        className="text-pink-600 hover:text-pink-700 ml-4 transition-colors"
                      >
                        التفاصيل
                      </button>
                    </td>
